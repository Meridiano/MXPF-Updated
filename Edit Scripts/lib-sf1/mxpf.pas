...
var
	...
	mxFileMode, mxRecordsCopied, mxRecordsFound, mxMasterRecords, mxOverrideRecords: Integer;
	mxInitialized, mxLoadCalled, mxCopyCalled, mxLoadMasterRecords, mxLoadOverrideRecords,
	mxLoadWinningOverrides, mxMastersAdded, mxSkipPatchedRecords, mxDisallowNewFile,
...
begin
	...
	if not (Assigned(mxPatchFile) or mxDisallowNewFile) then begin
		ShowMessage('Patch file "' + ChangeFileExt(filename, '') + '" not found and will be created.');
	...
	if not SameText(GetFileName(mxPatchFile), filename) then
		ShowMessage('Incorrect filename used. The script will not recognize this file as the patch in the future.');
...
begin
	...
	if mxRecords.Count = 0 then
		raise Exception.Create('MXPF Error: Not able to call GetRecord, no records available');
...
begin
	...
	// if no records available, raise exception
	if mxRecords.Count = 0 then
		raise Exception.Create('MXPF Error: Not able to call CopyRecordToPatch, no records available');
	// if index is out of bounds, raise an exception
	if (i < 0) or (i > MaxRecordIndex) then
...
begin
	...
	// if no records available, raise exception
	if mxRecords.Count = 0 then
		raise Exception.Create('MXPF Error: Not able to call CopyRecordsToPatch, no records available');
